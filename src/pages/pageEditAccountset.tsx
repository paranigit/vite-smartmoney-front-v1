...
export default function PageEditAccount() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      fetch(
        "https://3gqf36yau6b2bnuwg3x6y6w5bq0ohikb.lambda-url.eu-west-2.on.aws/?action=update",
        requestOptions
      )
  ...
  const getAccount = async () => {
    fetch(
      "https://3gqf36yau6b2bnuwg3x6y6w5bq0ohikb.lambda-url.eu-west-2.on.aws/?action=read&account_id=" +
        account_id
    )
      .then((response) => response.json())
      .then((jsondata) => {
        // Get the first occurance of accounts
        let accountInfo = jsondata["accounts"][0];

        // Convert the accountsets string to array.
        let accountsets: string[] = accountInfo.accountsets
          .split(",")
          .map((item: string) => item.trim());
        accountInfo["accountset"] = accountsets;

        // Update the account, accountsets values
        setSelectedAccountsets(accountsets);
  ...
  const RenderAccountForm = () => {
    return isLoading ? (
      ...
      <div className="py-5">
        <h4 className="py-3">Edit Account</h4>
